...
def generate_random_graph(n, extra_edges):
    ...
    while added < extra_edges:
        ...
        if u != v and not G.has_edge(u, v):
            G.add_edge(u, v, delay=random.uniform(0.1, 1), error=random.uniform(0.1, 1))
            added += 1
    return G
...
def solve_single_dual_bound(G, source, target, T, lambda_grid):
    ...
    for lam in lambda_grid:
        ...
        try:
            sp_cost = nx.shortest_path_length(G, source=source, target=target, weight='modified_cost')
            path = nx.shortest_path(G, source=source, target=target, weight='modified_cost')
...
def unconstrained_shortest_path(G, source, target, lambda_dict):
    ...
    try:
        cost = nx.shortest_path_length(G, source=source, target=target, weight='mod_cost')
        path = nx.shortest_path(G, source=source, target=target, weight='mod_cost')
    except nx.NetworkXNoPath:
        cost = float('inf')
        path = None
    return cost, path
...
# ------------------------------
def run_experiment(n=1000, extra_edges=40000, T=1,
