...
class DualSimulation(Simulation):
    ...
    def run_step(self):
        ...
        if self.VISUALIZE:            
            ...
            self._update_o_lisl(satp=connected_sat, viz=self.viz_list[0], binary=True)
            # self._update_o_lisl(satp=srouting[:,0:2].astype(np.int64), viz=self.viz_list[1], binary=True)
