...
lml_res = -lml_data[:, 3]
lml_time = lml_data[:, 2]/1e6

heu_data = np.genfromtxt(HEU_PATH, delimiter=',')
heu_res = -heu_data[:, 3]
heu_time = heu_data[:, 2]/1e6
...
l = axs.scatter(np.mean(heu_time), np.mean(heu_res), marker='D', color='blue', s=30, zorder=5)
lines1.append(l)
l = axs.scatter(np.mean(lml_time), np.mean(lml_res), marker='*', color='red', s=80, zorder=5)
lines1.append(l)
axs.set_position([0.15, 0.175, 0.8, 0.75])
axs.set_xlabel(r'Computation Time (s)')
axs.set_ylabel(r'Network Throughput (Gbps)')
axs.set_xlim(0, 500)
axs.set_xscale("symlog", linthresh=0.001)  # linear near 0, log away from 0
axs.set_ylim(70, 120)
