...
axs_list = [fig.add_subplot(3, 4, i+1) for i in range(12)]  # Create 3D subplots
...
for i, FOR in enumerate(FOR_list):
    for j, METHOD in enumerate(METHOD_list):
        data_file = os.path.join(results_dir, f"connected_s_t_for_{FOR}_seed_0_{METHOD}.csv")
        ...
        l = axs.scatter(x, y, c=data[:,2], label=f"FOR {FOR} - {METHOD}", s=5, cmap=c_map, vmin=0, vmax=0.5)
        lines1.append(l)
