...
l, = axs.plot(np.arange(500)+1, sg_res_avg, linestyle='-', linewidth=LINE_WIDTH)
sg_res_avg_ideal = -sg_data[:,4].reshape(-1, 500).mean(axis=0)
l, = axs.plot(np.arange(500)+1, sg_res_avg_ideal, linestyle=':', linewidth=LINE_WIDTH)
lines1.append(l)

#add heuristic and lml
l, = axs.plot(np.arange(500)+1, np.full(500, np.mean(heu_res)), linestyle='--', linewidth=LINE_WIDTH)
lines1.append(l)
l, = axs.plot(np.arange(500)+1, np.full(500, np.mean(lml_res)), linestyle='-.', linewidth=LINE_WIDTH)
lines1.append(l)

axs.set_position([0.135, 0.18, 0.35, 0.65])
...
# data_name_list = [r"SG-$K=5$", r"SG-$K=10$", r"SG-$K=20$", r"SG-$K=50$", r"SG-$K=100$", r"MRate", r"LML"]
...
# leg = axs.legend(h,l,fontsize=FONT_SIZE, loc='lower left', bbox_to_anchor=(0.65, 0.025, 0.325, 0.3), mode="expand",ncol = 1 ,borderaxespad=0.,handlelength=1, handleheight= 0.8, handletextpad=0.2, 
...
axs = axs_list[1]
lines1 = []
sg_time_avg = sg_data[:,2].reshape(-1, 500).mean(axis=0)/1e6  #convert to seconds
l, = axs.plot(np.arange(500)+1, sg_time_avg, linestyle='-', linewidth=LINE_WIDTH)
lines1.append(l)
sg_time_avg_ideal = np.zeros_like(sg_time_avg)
l, = axs.plot(np.arange(500), sg_time_avg_ideal, linestyle=':', linewidth=LINE_WIDTH)
lines1.append(l)

#add heuristic and lml
l, = axs.plot(np.arange(500)+1, np.full(500, np.mean(heu_time))/1e6, linestyle='--', linewidth=LINE_WIDTH)
lines1.append(l)
l, = axs.plot(np.arange(500)+1, np.full(500, np.mean(lml_time))/1e6, linestyle='-.', linewidth=LINE_WIDTH)
lines1.append(l)

axs.set_position([0.625, 0.18, 0.35, 0.65])
axs.set_xlabel(r'Number of Iterations, $K$')
axs.set_ylabel(r'Computation Time (s)')
# axs.set_xlim(50, 150)
axs.set_ylim(-0.01, 1000)
axs.set_xscale('log')
axs.set_yscale("symlog", linthresh=1e-2)  # linear near 0, log away from 0
axs.grid(True, zorder=0)
# axs.text(20, -5000, r'$\bf{(a)}$', fontsize=FONT_SIZE+2, verticalalignment='bottom')

data_name_list = [r"SG", r"SG-Ideal", r"MRate", r"LML"]
ncol = 1  # Number of columns in the legend
h, l = lines1, data_name_list
# nrows = -(-len(h) // ncol)                         # ceiling division
# idx   = np.arange(len(h))
...
leg = fig.legend(h,l,fontsize=FONT_SIZE, loc='lower left', bbox_to_anchor=(0.135, 0.875, 0.84, 0.125), mode="expand",ncol = 4 ,borderaxespad=0.,handlelength=2, handleheight= 0.8, handletextpad=1, 
frameon=True,          # draw a frame
fancybox=False,        # <-- square corners (like MATLAB)
edgecolor='black',     # black  frame edge
facecolor='white',     # white background
