...
N_POINTS = 2
results_file = "/home/zhouyou/leo-sat-flow/sim_alg_j1_res/test_ld_starlink_1000_varying_for/test_ld_starlink_1000_varying_for-2025-November-27-13-36-06-ail/ldl"
...
data = data.mean(axis=1).reshape(-1, 5)
print(data)
markers = ['o', 's', 'D', '^', 'v', 'x', '*']
axs = axs_list[0]
bar_width = 0.325
...
axs.set_xticklabels(["30", "40", "50", "60", "70", "80", "90"])
...
name = ["DeepLaDu","+Grid","Rand","MRate", "SaTE"]
data = np.genfromtxt(results_file, delimiter=',')
data = -data[:, [4,7,8,6,9]]  # Select the relevant columns and negate them
data = data.reshape(7,N_POINTS, 5)
data = data.mean(axis=1).reshape(-1, 5)
...
index = np.arange(data.shape[0])*2
for i in range(data.shape[1]):
    b = axs.bar(index + (i -2)* bar_width, data[::-1, i], bar_width, label=name[i], zorder=3)
    bars.append(b)
...
leg = fig.legend(bars, name ,fontsize=FONT_SIZE, loc='lower left', bbox_to_anchor=(0.125, 0.9, 0.865, 0.125), mode="expand",ncol = 5 ,borderaxespad=0.,handlelength=1, handleheight= 0.8, handletextpad=0.2, 
