...
results_file = "/home/zhouyou/leo-sat-flow/sim_alg_v1_res/test_dual_optimization_mixed_constellation/test_dual_optimization_mixed_constellation-2025-July-19-17-51-44-ail/res.csv"

# Plot the data
fig, axs = plt.subplots(2,2)
fig.set_size_inches(fig_width_in, fig_height_in)  # 3.5 inches width, height adjusted to maintain aspect ratio

name = ["+Grid,OSPF","+Grid,SPF",  "MRate,OSPF", "MRate,SPF"]
data = np.genfromtxt(results_file, delimiter=',')
for i in range(4):
    our_data = data[:, 4]
    data_arrays = data[:, 5+i]
    ratio = our_data/data_arrays
    ratio = ratio.reshape((7,11))
    print(ratio)
    # ratio = np.clip(ratio, 1, )  # Clip values to the range [vmin, vmax]
    print(axs[i%2, i//2].get_position())
    im = axs[i%2, i//2].imshow(ratio, cmap='viridis', aspect='auto')
    axs[i%2, i//2].set_title(f"Ratio to {name[i]}")
    axs[i%2, i//2].set_xticks([0,5,10])
    axs[i%2, i//2].set_yticks(np.arange(7))
    axs[i%2, i//2].set_xticklabels(np.asarray([0,5,10])/10.)
    axs[i%2, i//2].set_yticklabels(np.arange(7)*250+500)

cbar =fig.colorbar(im)
axs[1, 1].set_xlabel('Regularity')
axs[0, 0].set_ylabel('Number of Satellites')
...
# leg = plt.legend(h,l,fontsize=FONT_SIZE, loc='lower left', bbox_to_anchor=(0.125, 0.025, 0.7, 0.3), mode="expand",ncol = 3 ,borderaxespad=0.,handlelength=1, handleheight= 0.8, handletextpad=0.2, 
