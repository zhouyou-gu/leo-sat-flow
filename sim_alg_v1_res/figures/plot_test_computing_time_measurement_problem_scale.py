...
N_POINTS = 20
res = np.zeros((len(n_sat), 6, N_POINTS))
result_list = []
for i, n in enumerate(n_sat):    
    print(f"Processing n_sat = {n}")
    tmp_dir = "test_computing_time_measurement-2025-July-23-19-14-35-ail"
    fname = f"lpdsolver.n_pe.n_sat{n}.txt"
    data_file = os.path.join(results_dir, tmp_dir, fname)
    data = np.genfromtxt(data_file, delimiter=',')
    res[i, 0, :] = data[:, 3]
    
    fname = f"lpdsolver.n_pp.n_sat{n}.txt"
    data_file = os.path.join(results_dir, tmp_dir, fname)
    data = np.genfromtxt(data_file, delimiter=',')
    res[i, 1, :] = data[:, 3]

    result_list.append(data[:, 3])
    fname = f"lpdsolver.n_st.n_sat{n}.txt"
    data_file = os.path.join(results_dir, tmp_dir, fname)
    data = np.genfromtxt(data_file, delimiter=',')
    res[i, 2, :] = data[:, 3]

    fname = f"lpdsolver.n_cp.n_sat{n}.txt"
    data_file = os.path.join(results_dir, tmp_dir, fname)
    data = np.genfromtxt(data_file, delimiter=',')
    res[i, 3, :] = data[:, 3]

    # fname = f"lpdsolver.n_ce.n_sat{n}.txt"
    # data_file = os.path.join(results_dir, tmp_dir, fname)
    # data = np.genfromtxt(data_file, delimiter=',')
    # res[i, 4, :] = data[:, 3]

    fname = f"lpdsolver.n_fp.n_sat{n}.txt"
    data_file = os.path.join(results_dir, tmp_dir, fname)
    data = np.genfromtxt(data_file, delimiter=',')
    res[i, 4, :] = data[:, 3]
...
axs.set_ylim(100, 1e5)
# axs.set_xscale('log')
axs.set_yscale('log')
axs.grid()
# Add a legend

data_name_list = [r'$|\mathcal{E}|$',r'$|\mathcal{L}|$', r'$|\mathcal{F}|$',r"$|\mathcal{L}'|$",r"$|\mathcal{F}'|$"]
...
leg = fig.legend(h,l,fontsize=FONT_SIZE, loc='lower left', bbox_to_anchor=(0.15, 0.875, 0.8, 0.15), mode="expand",ncol = 5 ,borderaxespad=0.,handlelength=1, handleheight= 0.8, handletextpad=0.4, 
